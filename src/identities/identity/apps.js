...
import { ORBITDB_APPS_STORE_NAME, ORBITDB_APPS_DEVICES_STORE_NAME, DB_TYPE } from './utils/constants/apps';
...
const loadOrbitdbStores = async (replicatedDb) => {
    const appsStore = await replicatedDb.loadStore(ORBITDB_APPS_STORE_NAME, DB_TYPE);
    const appsDevicesStore = await replicatedDb.loadStore(ORBITDB_APPS_DEVICES_STORE_NAME, DB_TYPE);
...
export const createApps = async (currentDeviceId, identityDescriptor, replicatedDb) => {
    const { appsStore, appsDevicesStore } = await loadOrbitdbStores(replicatedDb, identityDescriptor.id);

    return new Apps(currentDeviceId, identityDescriptor, appsStore, appsDevicesStore);
};

export const removeApps = async (identityDescriptor, replicatedDb) => {
    await replicatedDb.dropStore( ORBITDB_APPS_STORE_NAME, DB_TYPE);
    await replicatedDb.dropStore( ORBITDB_APPS_DEVICES_STORE_NAME, DB_TYPE);
};
