...
import { DB_NAME, DB_TYPE, DID_PUBLIC_KEY_PREFIX, DESCRIPTOR_SENSITIVE_KEYS } from './utils/constants/devices';
...
class Devices {
    ...
    async revoke(id, params) {
        ...
        if (!device.revokedAt) {
            // Remove device from DID Document and only then remove it from replicatedDb
...
export const createDevices = async (currentDevice, identityDescriptor, didm, storage, replicatedDb) => {
    ...
    const orbitdbStore = await replicatedDb.loadStore(DB_NAME);
...
export const restoreDevices = async (identityDescriptor, didm, storage, replicatedDb) => {
...
export const removeDevices = async (identityDescriptor, storage, replicatedDb) => {
    await storage.remove(getCurrentDeviceKey(identityDescriptor.id));
    await replicatedDb.dropStore( DB_NAME, DB_TYPE);
};
