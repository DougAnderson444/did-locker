...
class Identities {
    ...
    constructor(storage, didm, linkage) {

        this.#storage = storage;
        this.#didm = didm;
        this.#linkage = linkage; // database to link profiles on multiple devices
    }
    ...
    async load() {
        ...
        if (!this.#identitiesLoad) {
            this.#identitiesLoad = identityFns.loadIdentities(this.#storage, this.#didm, this.#linkage);
    ...
    async peek(didMethod, params) {
        ...
        const profileDetails = identity ? identity.profile.getDetails() : await identityFns.peekProfileDetails(did, this.#linkage);
    ...
    async create(didMethod, params) {
        ...
        const identity = await identityFns.createIdentity({
            ...
            backupData,
            profileDetails,
        }, this.#storage, this.#didm, this.#linkage);
    ...
    async remove(id, params) {
        ...
        await identityFns.removeIdentity(id, this.#storage, this.#linkage);
...
const createIdentities = (storage, didm, linkage) => new Identities(storage, didm, linkage);
